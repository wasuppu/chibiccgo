...
func NewVlaPtr(vara *Obj, tok *Token) *Node {
...
func declaration(rest **Token, tok *Token, basety *Type, attr *VarAttr) *Node {
	...
	for !tok.equal(";") {
		...
		if ty.kind == TY_VLA {
			...
			tok := ty.name
			expr := NewBinary(ND_ASSIGN, NewVlaPtr(vara, tok),
...
func arrayDesignator(rest **Token, tok *Token, ty *Type, begin, end *int) {
	*begin = int(constExpr(&tok, tok.next))
	if *begin >= ty.arrayLen {
		failTok(tok, "array designator index exceeds array bounds")
	}

	if tok.equal("...") {
		*end = int(constExpr(&tok, tok.next))
		if *end >= ty.arrayLen {
			failTok(tok, "array designator index exceeds array bounds")
		}
		if *end < *begin {
			failTok(tok, "array designator range [%d, %d] is empty", *begin, *end)
		}
	} else {
		*end = *begin
	}
...
func toAssign(binary *Node) *Node {
	...
	if binary.lhs.ty.isAtomic {
		...
		cur.next = NewUnary(ND_EXPR_STMT,
...
// | "default" ":" assign
func genericSelection(rest **Token, tok *Token) *Node {
	start := tok
	tok = tok.skip("(")
	...
	if ret == nil {
		failTok(start, "controlling expression type not compatible with"+
...
func primary(rest **Token, tok *Token) *Node {
	...
	if tok.kind == TK_IDENT {
		...
		*rest = tok.next
		...
		if sc != nil && sc.vara != nil && sc.vara.isFunction {
			if currentParseFn != nil {
				currentParseFn.refs = append(currentParseFn.refs, sc.vara.name)
			} else {
				sc.vara.isRoot = true
			}
		}
...
func markLive(vara *Obj) {
	if !vara.isFunction || vara.isLive {
		return
	}
	vara.isLive = true

	for i := 0; i < len(vara.refs); i++ {
		fn := findFunc(vara.refs[i])
		if fn != nil {
			markLive(fn)
		}
	}
...
func scanGlobals() {
	...
	for vara := globals; vara != nil; vara = vara.next {
		...
		vara2 := globals
		for ; vara2 != nil; vara2 = vara2.next {
			if vara != vara2 && vara2.isDefinition && vara.name == vara2.name {
				break
			}
		}
